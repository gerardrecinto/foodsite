...
def simplex(indexes):
  ...
  data = [
    ...
    ['Wheat Cereal', '28 oz.', 24.2, 11.8, 377, 14.4, 175, 0, 14.4, 8.8, 114, 0],
    ['Corn Flakes', '8 oz.', 7.1, 11.4, 252, 0.1, 56, 0, 13.5, 2.3, 68, 0],
    ['Corn Meal', '1 lb.', 4.6, 36.0, 897, 1.7, 99, 30.9, 17.4, 7.9, 106, 0],
    ['Hominy Grits', '24 oz.', 8.5, 28.6, 680, 0.8, 80, 0, 10.6, 1.6, 110, 0],
    ['Rice', '1 lb.', 7.5, 21.2, 460, 0.6, 41, 0, 2, 4.8, 60, 0],
    ['Rolled Oats', '1 lb.', 7.1, 25.3, 907, 5.1, 341, 0, 37.1, 8.9, 64, 0],
    ['White Bread', '1 lb.', 7.9, 15.0, 488, 2.5, 115, 0, 13.8, 8.5, 126, 0],
    ['Whole Wheat Bread', '1 lb.', 9.1, 12.2, 484, 2.7, 125, 0, 13.9, 6.4, 160, 0],
    ['Rye Bread', '1 lb.', 9.1, 12.4, 439, 1.1, 82, 0, 9.9, 3, 66, 0],
    ['Pound Cake', '1 lb.', 24.8, 8.0, 130, 0.4, 31, 18.9, 2.8, 3, 17, 0],
    ['Soda Crackers', '1 lb.', 15.1, 12.5, 288, 0.5, 50, 0, 0, 0, 0, 0],
    ['Milk', '1 qt.', 11, 6.1, 310, 10.5, 18, 16.8, 4, 16, 7, 177],
    ['Evaporated Milk', '14.5 oz.', 6.7, 8.4, 422, 15.1, 9, 26, 3, 23.5, 11, 60],
    ...
    ['Salmon', '16 oz.', 13, 5.8, 705, 6.8, 45, 3.5, 1, 4.9, 209, 0],
  ...
  for i in range(0,numChoices):
      if 'lb' in data[indexes[i]][1] or 'No.' in data[indexes[i]][1]:
          calories[i] = 3*1/(4.0/3+numChoices)
      else:
          calories[i] = min(.2,3*1/(4.0/3+numChoices))
  ...
  constraints = [0] * (len(nutrients)+numChoices)
  ...
  for i in range(0,numChoices):
    constraints[len(nutrients)+i] = solver.Constraint(calories[i], solver.infinity())
    constraints[len(nutrients)+i].SetCoefficient(food[indexes[i]], data[1][3])
  ...
  if status == solver.OPTIMAL:
    ...
    for i in range(0, len(data)):
      ...
      if food[i].solution_value() > 0:
        ans.append((data[i][0],round(food[i].solution_value()*18,2)))
    return ans
